...
class PH:
    ...
    def __init__(self,
        ...
        self.is_density = bool(is_density)    
    ...
    # ------------- Helper: drop points in prohibited region ----------
    def _filter_excluded_points(self, pts: torch.Tensor) -> torch.Tensor:
        ...
        # Mark a point as "inside" if it coincides with any inside_points row (within tolerance)
        N, D = pts.shape
    ...
    # ---------- PH compute (envelope-only grid -> Cripser) ----------
    def get_PH(self):
    ...
    def connectedness_loss_from_density(self,
        ...
        # ----- 1) Build PH on g(x) = iso_level - rho(x) (no grad here) -----
        ...
        with torch.no_grad():
            ...
            g_in = iso_level - rho_in  # signed field; g=0 at rho=iso_level
...
def make_inside_envelope_fn_from_domain(domain, nx: int, device=None):
    ...
    def _fn(x):
        # Normalize to a torch tensor on the desired device
